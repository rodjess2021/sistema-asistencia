...
if (!empty($_GET["txtFechainicio"])  and !empty($_GET["txtFechafinal"]) and !empty($_GET["txtAsesor"])) {
   ...
   class PDF extends FPDF
   {
      ...
      function Header()
      {
         ...
         $this->Cell(100, 10, utf8_decode("REPORTE TOTAL DE HORAS DE SEGUNDO BAÑO "), 0, 1, 'C', 0);
         $this->Ln(7);
   ...
   date_format(asistencia.`2Sshh_inicio`, '%d-%m-%Y %H:%i:%s') as '2Sshh_inicio',
   date_format(asistencia.`2Sshh_fin`, '%d-%m-%Y %H:%i:%s') as '2Sshh_fin',
   TIMEDIFF(asistencia.`2Sshh_fin`, asistencia.`2Sshh_inicio`) as 'totalHR',
   empleado.nombre,
   empleado.apellido,
   empleado.dni,
...
WHERE
   asistencia.`2Sshh_inicio` BETWEEN '$fechaInicio' and '$fechaFinal'
ORDER BY
   ...
   date_format(asistencia.`2Sshh_inicio`, '%d-%m-%Y %H:%i:%s') as '2Sshh_inicio',
   date_format(asistencia.`2Sshh_fin`, '%d-%m-%Y %H:%i:%s') as '2Sshh_fin',
   TIMEDIFF(asistencia.`2Sshh_fin`, asistencia.`1Sshh_inicio`) as 'totalHR',
   empleado.nombre,
   empleado.apellido,
   empleado.dni,
   cargo.nombre AS 'cargo'
...
INNER JOIN  cargo ON empleado.cargo = cargo.id_cargo
WHERE
   asistencia.id_empleado = $empleado AND asistencia.`2Sshh_inicio` BETWEEN '$fechaInicio' and '$fechaFinal'
ORDER BY
   asistencia.id_empleado ASC");
   }
   ...
   while ($datos_reporte = $sql->fetch_object()) {
      ...
      $pdf->Cell(50, 10, utf8_decode($datos_reporte->{"2Sshh_inicio"}), 1, 0, 'C', 0);
      $pdf->Cell(50, 10, utf8_decode($datos_reporte->{"2Sshh_fin"}), 1, 0, 'C', 0);
      $pdf->Cell(30, 10, utf8_decode($datos_reporte->totalHR), 1, 1, 'C', 0);
   }


   $pdf->Output('Reporte Segundo Baño.pdf', 'I'); //nombreDescarga, Visor(I->visualizar - D->descargar)
}
